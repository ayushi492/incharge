...
<hr>-->
    ...
    {
        if($rows['add_site_region']==1)
        { ?>
	    <div style="float:right;"><a href="index.php?action=add_site_region"><button type="button" class="btn btn-gradient-primary" title="" data-toggle="tooltip"><i class="feather icon-plus"></i>Add Site Region</button></a></div>
    <?php
        }
    }
    else{
        echo'<div style="float:right;"><a href="index.php?action=add_site_region"><button type="button" class="btn btn-gradient-primary" title="" data-toggle="tooltip"><i class="feather icon-plus"></i>Add Site Region</button></a></div>';
    }
    if(isset($_REQUEST['a']))
	  {
		  ?>
	...
	<table class="table table-bordered">
		<thead style="color:#fff;">
		  <tr style="color:#fff;">
			...
			<th bgcolor="#38AAFC" style="width:44%;">Site Region </th>
            <th bgcolor="#38AAFC" style="width:44%;">Descriprtion </th>
            <?php
            if($_SESSION['incharge_ovp']!='admin')
            {
              if($rows['edit_site_region']==1)
...
<?php
		...
		$getRecordQryNum=mysqli_num_rows($GetDetail3);
		...
		//echo "hh";
        $ctr = 1;
		while($row = mysqli_fetch_assoc($GetDetail3))
		{
			$class = '';
			if($ctr%2 == 0)
			{
...
<?php
              ...
              if($rows['edit_site_region']==1)
              { ?>
                  <td><a class="green tooltipt" href="index.php?action=edit_site_region&id=<?php echo $row['id'];?>" style="text-decoration:none;"><span class="tooltiptext"><i class="fas fa-edit"></i> <span class="tooltip-top"></span></span> <i class="ace-icon fa fa-pencil bigger-130"></i> </a> &nbsp;&nbsp;&nbsp;
                      <?php
              }
                      }
              else{ ?>
...
?>                                                      
	  ...
	  <?php
		...
		$targetpage="index.php?action=view_site_region";
		#PAGINATION CODE
